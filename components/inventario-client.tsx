...
export function InventarioClient({ organizationId, initialAssets }: { organizationId: string; initialAssets: Asset[] }) {
  ...
  return <div className='space-y-4'><PageHeader title='Activos' description='Control de activos físicos del billar: tacos, bolas, dominó, refrigeradores y más.' /><div className='flex gap-2'><input className='rounded bg-rack-shell/70 p-2' value={name} onChange={e=>setName(e.target.value)} placeholder='Nombre activo' /><button className='rounded border px-3' onClick={add}>Agregar</button></div>{assets.map(a=><div key={a.id} className='rounded border p-3'>{a.name} · {a.asset_type} · {a.quantity} · {a.status}</div>)}</div>;
}
